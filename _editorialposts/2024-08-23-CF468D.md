...
Let us assume that we know the node $c$ which is to be covered by all paths for now and worry about neither lexicographic minimization, nor maximization of $\sum_{1 \leq u \leq n} {\text{dis(c, u)}}$ for now. Let's just focus on finding some valid permutation.
...
a. We don't know what $c$ is.
b. We have to maximise  $\sum_{1 \leq u \leq n} {\text{dis(c, u)}}$.
c. We have to lexicographically minimize $p$.
