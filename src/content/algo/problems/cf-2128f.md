...
To optimize this to $O((n + m) \log{n})$, we can simply observe that we can use a Djikstra's like approach here due to the monotonic nature of the transitions (due to strictly positive edge weights).