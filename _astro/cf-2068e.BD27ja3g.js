...
- For every node, compute the lowest in-time cross-edge, back-edge, and greatest in-time cross-edge, back-edge. 
- Build a merge sort tree on the euler tour such that for every node $u$, you store the aforementioned four things (in the form of pairs - $(\\operatorname{time}, \\operatorname{level}_u)$, where $\\operatorname{time}$ is one of the four times) at the leaf-node in the merge-sort tree at the in-time of $u$. The rest of the merge-sort tree is built using the pairs at these leaf nodes.
