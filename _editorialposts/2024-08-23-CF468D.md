...
> You are given a complete bipartite graph $G(V, E)$ (defined on *equally* sized partitions $A$ and $B$), and some groups of nodes $S_1, S_2, \dots S_x$ such that $S_i \subseteq A \cup B$, $S_i \cap S_j = \emptyset \; \forall \; i \neq j$ and $S_1 \cup S_2 \dots \cup S_x = B$. Find a perfect matching on $G$ such that there is no edge between two nodes which belongs to the same group of nodes $S_i$.

Note: In our problem, we have the set of permutation indices as $A$, the set of permutation values as $B$, and the groups of nodes are formed by partitioning nodes on the basis of $\text{rep_u}$. 
