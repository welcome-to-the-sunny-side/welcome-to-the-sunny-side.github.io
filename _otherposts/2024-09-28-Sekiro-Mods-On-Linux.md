...
- Edit modengine.ini (which you pasted in step 1) and set the variable `modOverrideDirectory` "\ftsoa" (or whatever you named it to).
- Open Winetricks for the relevant wineprefix, go to `library`, and add an override for `dinput8.dll`.
- Now, launch Lutris, go to Sekiro's configuration, and set the working directory to the same directory where sekiro.exe is located.
- Enjoy.